...
_SUMMARY_KEYS = ("cumulative_tok_s", "decode_tok_s", "prefill_tok_s", "ttft_s", "total_s")
...
async def one(client, url, model, max_tokens, think_kwarg, think_val):
    body = {
        "model": model,
...
async def _stream(client, url, body, t0):
    ttft = None
    usage = None
    ntok = 0
...
async def one_retry(client, url, model, max_tokens, think_kwarg, think_val, attempts=6, label=""):
...
def summarize(runs):
    ...
    out = {"n_ok": len(ok), "n_err": len(runs) - len(ok)}
