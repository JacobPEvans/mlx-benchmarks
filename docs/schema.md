...
| `readiness` | object | First-request evidence kept outside the warmed score: `initial_model_state` is `cold`, `resident`, or `unknown`; `first_request` archives the discarded request's timings when available. |
