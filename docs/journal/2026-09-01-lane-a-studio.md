...
- Any 35B latency figure under vllm-mlx. All requests were rejected before
  reaching the model.
- Any cause for the vllm-mlx rejections. What was measured is that the backend
  answered a bypassing request while the gate rejected everything; why the gate
  did so was not determined here.
